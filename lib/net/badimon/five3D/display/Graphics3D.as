...
package net.badimon.five3D.display {
	...
	public final class Graphics3D {
		...
		private function draw(graphics:Graphics, matrix:Matrix3D, viewDistance:Number):void {
			var point1:Vector3D, point2:Vector3D, perspective:Number;
			graphics.clear();
			...
			for (var i:int = 0;i < len; i++) {
				switch (__motif[i][0]) {
					...
					case "S":
						...
						break;
					case "M":
						point1 = (__motif[i][1].length == 2) ? new Vector3D(__motif[i][1][0], __motif[i][1][1]) : new Vector3D(__motif[i][1][0], __motif[i][1][1], __motif[i][1][2]);
					...
					case "C":
						if (__motif[i][1].length == 4) {
							point1 = new Vector3D(__motif[i][1][0], __motif[i][1][1]);
							point2 = new Vector3D(__motif[i][1][2], __motif[i][1][3]);
						} else {
							point1 = new Vector3D(__motif[i][1][0], __motif[i][1][1], __motif[i][1][2]);
							point2 = new Vector3D(__motif[i][1][3], __motif[i][1][4], __motif[i][1][5]);
						}
						point1 = matrix.transformVector(point1);
